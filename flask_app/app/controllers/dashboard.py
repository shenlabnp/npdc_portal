...
def page_dashboard():
    ...
    ),  sqlite3.connect(conf["user_db_path"], timeout=60)).iloc[0, 0]


    num_login_today = pd.read_sql_query((
        "select count(id) from users where last_login >= ?"
    ),  sqlite3.connect(conf["user_db_path"], timeout=60), params=(datetime.now().strftime("%Y-%m-%d 00:00:00"),)).iloc[0, 0]
    ...
    ),  sqlite3.connect(conf["user_db_path"], timeout=60)).iloc[:, 0]
    ...
    users_jobs_academia = pd.read_sql_query((
        ...
        " group by name"
    ),  sqlite3.connect(conf["user_db_path"], timeout=60)).values.tolist()
    ...
    ),  sqlite3.connect(conf["query_db_path"], timeout=60)).values.tolist()
